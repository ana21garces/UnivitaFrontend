...
import { ChevronDown, ChevronUp, Users, Dumbbell, AlertCircle, Building2, GraduationCap, Bell, Check, XCircle, FileDown } from "lucide-react"
...
type ActFisicaData = { total_usuarios: number; facultades: Facultad[] }
...
// El nivel llega de af_nivel, calculado por el backend con los cortes reales
// del PEPS II (≤25/≤50/≤75). Antes se recalculaba aquí con ≤33/≤55/≤77 -otra
// escala inventada- y un usuario con índice 30 se veía "Pobre" en vez de
// "Moderado".
...
function UsuarioRow({
  ...
  const af = usuario.actividad_fisica
  const requiereAtencion = af.af_nivel === "Pobre"
  const retrocedio = usuario.indice_anterior != null && af.af_indice < usuario.indice_anterior
  const necesitaCita = requiereAtencion || af.af_nivel === "Moderado" || retrocedio
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <ComparativoAnterior actual={af.af_indice} anterior={usuario.indice_anterior} />
          </div>
        </div>
        <div className="flex items-center gap-3 ml-4">
          <div className="hidden sm:flex items-center gap-2 w-52"><IndiceBar indice={af.af_indice} nivel={af.af_nivel} /></div>
      ...
      {open && (
        <div className="px-4 pb-4 pt-2 bg-[#F8FAFC] border-t border-[#E2E8F0]">
          <div className="sm:hidden mb-3">
            ...
            <IndiceBar indice={af.af_indice} nivel={af.af_nivel} />
          </div>
            ...
            {AF_ITEMS.map((item) => {
              ...
              return (
                <div key={item} className="flex items-center gap-3 bg-white border border-[#E2E8F0] rounded-lg px-3 py-2">
                  <span className="text-xs font-bold text-[#6B7280] w-14 shrink-0">Ítem {item.replace("af_item_", "")}</span>
              ...
              <button
                ...
                onClick={() =>
                  onReporte({
                    ...
                    preguntas: AF_ITEMS.map((item) => ({
                      numero: item.replace("af_item_", ""),
                      texto: AF_ITEM_TEXTO[item],
                      valor: af[item],
                    })),
                  })
                }
...
function FiltrosBar({ filtros, onChange, opciones }: {
  ...
  const sc = "text-sm border border-[#E2E8F0] rounded-lg px-3 py-1.5 bg-white text-[#1F2937] focus:outline-none focus:border-[#2563EB] cursor-pointer disabled:opacity-50"
  return (
...
export default function ActividadFisicaPage() {
  useTituloPagina("Actividad física")
  ...
  useEffect(() => {
    ...
    if (!token) return
    api
      .get("/encuesta/actividad-fisica/resultados/estadisticas")
  ...
  useEffect(() => {
    const token = getToken()
    if (!token) return
    api.get("/notificaciones/notificados?rol=actividad_fisica")
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 flex flex-col gap-6">
        ...
        {stats && <EstadisticasSection stats={stats} tituloDimension="Actividad física" />}
      ...
      {notifObjetivo && (
        <NotificarModal
          ...
          mensajeSugerido="Te invitamos a agendar una cita con el profesional de actividad física para hablar de tus resultados."
          rol="actividad_fisica"
      ...
      {reporteObjetivo && (
        <ReporteIndividualModal
          ...
          dimensionClave="actividad_fisica"
          dimensionLabel="Actividad Física"
