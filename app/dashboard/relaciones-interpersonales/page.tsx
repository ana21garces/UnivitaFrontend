...
import { ChevronDown, ChevronUp, Users, HeartHandshake, AlertCircle, Building2, GraduationCap, Bell, Check, XCircle, FileDown } from "lucide-react"
...
type RelacionesInterpersonales = {
  ri_item_01: number
  ri_item_07: number
  ri_item_13: number
  ri_item_20: number
...
const RI_ITEM_TEXTO: Record<string, string> = {
  ri_item_01: "Dialogar sus problemas o preocupaciones con personas muy allegadas",
  ri_item_07: "Elogiar a otras personas por sus éxitos",
  ri_item_13: "Mantener relaciones sociales positivas",
  ri_item_20: "Dedicar tiempo para estar con sus amistades más cercanas",
  ri_item_26: "Expresar amor o cariño a otros",
  ri_item_32: "Utilizar el toque físico con las personas importantes",
  ri_item_38: "Buscar maneras respetuosas de llenar sus necesidades afectivas",
  ri_item_45: "Buscar apoyo en las personas que se preocupan por él/ella",
  ri_item_50: "Comprometerse a encontrar puntos en común, por medio del diálogo, con otras personas",
}
...
// El nivel llega de ri_nivel, calculado por el backend con los cortes reales
...
function UsuarioRow({
  ...
  const ri = usuario.relaciones_interpersonales
  const requiereAtencion = ri.ri_nivel === "Pobre"
  const retrocedio = usuario.indice_anterior != null && ri.ri_indice < usuario.indice_anterior
  const necesitaCita = requiereAtencion || ri.ri_nivel === "Moderado" || retrocedio
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <ComparativoAnterior actual={ri.ri_indice} anterior={usuario.indice_anterior} />
          </div>
        </div>
        <div className="flex items-center gap-3 ml-4">
          <div className="hidden sm:flex items-center gap-2 w-52"><IndiceBar indice={ri.ri_indice} nivel={ri.ri_nivel} /></div>
      ...
      {open && (
        <div className="px-4 pb-4 pt-2 bg-[#F8FAFC] border-t border-[#E2E8F0]">
          <div className="sm:hidden mb-3">
            ...
            <IndiceBar indice={ri.ri_indice} nivel={ri.ri_nivel} />
          </div>
          <div className="flex flex-col gap-1.5 mb-3">
            {RI_ITEMS.map((item) => {
              const puntaje = ri[item]
              ...
              return (
                <div key={item} className="flex items-center gap-3 bg-white border border-[#E2E8F0] rounded-lg px-3 py-2">
                  <span className="text-xs font-bold text-[#6B7280] w-14 shrink-0">Ítem {item.replace("ri_item_", "")}</span>
                  <span className="flex-1 text-xs text-[#1F2937] leading-tight">{RI_ITEM_TEXTO[item]}</span>
              ...
              <button
                ...
                onClick={() =>
                  onReporte({
                    ...
                    preguntas: RI_ITEMS.map((item) => ({
                      numero: item.replace("ri_item_", ""),
                      texto: RI_ITEM_TEXTO[item],
                      valor: ri[item],
                    })),
                  })
                }
...
export default function RelacionesInterpersonalesPage() {
  useTituloPagina("Relaciones interpersonales")
  ...
  useReporteEnlace(!!data, (id) => {
    ...
    setReporteObjetivo({
      usuarioId: id,
      preguntas: RI_ITEMS.map((item) => ({ numero: item.replace("ri_item_", ""), texto: RI_ITEM_TEXTO[item], valor: u.relaciones_interpersonales[item] })),
    })
  })
  ...
  useEffect(() => {
    ...
    if (!token) return
    api
      .get("/encuesta/relaciones-interpersonales/resultados/estadisticas")
  ...
  const fetchData = useCallback((f: Filtros) => {
    ...
    const url = `/encuesta/relaciones-interpersonales/resultados${params.toString() ? `?${params}` : ""}`
    api.get(url)
      .then((res) => {
        const d: RIData = res.data
        setData(d)
  ...
  useEffect(() => {
    const token = getToken()
    if (!token) return
    api.get("/notificaciones/notificados?rol=relaciones_interpersonales")
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <DashboardNavbar role="relaciones-interpersonales" />
      <VolverAlPanelAdmin />
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold font-heading text-[#1F2937]">Relaciones Interpersonales</h1>
        ...
        {stats && <EstadisticasSection stats={stats} tituloDimension="Relaciones interpersonales" />}
      ...
      {notifObjetivo && (
        <NotificarModal
          ...
          mensajeSugerido="Te invitamos a agendar una cita con relaciones interpersonales para hablar de tus resultados."
          rol="relaciones_interpersonales"
      ...
      {reporteObjetivo && (
        <ReporteIndividualModal
          ...
          dimensionClave="relaciones_interpersonales"
          dimensionLabel="Relaciones Interpersonales"
