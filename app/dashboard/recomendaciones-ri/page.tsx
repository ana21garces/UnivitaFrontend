...
export default function RecomendacionesRIPage() {
  useTituloPagina("Plan · Relaciones interpersonales")
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 flex flex-col gap-6">
        ...
        {noEncuesta && (
          <div className="flex flex-col items-center gap-4 py-16 text-center">
            <HeartHandshake className="w-12 h-12 text-[#CBD5E1]" />
            <div>
              <p className="font-semibold text-[#1F2937]">Completa la encuesta primero</p>
              <p className="text-sm text-[#6B7280] mt-1">
                Debes completar el cuestionario de salud para ver tus recomendaciones de relaciones interpersonales.
              </p>
            </div>
            <Link
          ...
          <>
            {data.tarjetas.length === 0 ? (
              <div className="text-center py-16 text-[#6B7280]">
                ...
                <p className="text-sm">¡Excelente! No tienes áreas de mejora en relaciones interpersonales.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                {data.tarjetas.map((t) => (
