...
import { ArrowLeft, ChevronDown, ChevronUp, AlertCircle, Salad } from "lucide-react"
...
function TarjetaN({ tarjeta }: { tarjeta: Tarjeta }) {
...
export default function RecomendacionesNPage() {
  ...
  const [data, setData] = useState<RecomendacionesNData | null>(null)
  ...
  useEffect(() => {
    ...
    api
      .get("/encuesta/recomendaciones/nutricion")
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 flex flex-col gap-6">
          ...
          {data && (
            <div className="flex items-center gap-3 self-start sm:self-auto">
              <div className="px-3 py-1.5 rounded-xl bg-white border border-[#E2E8F0] shadow-sm text-center">
                <p className="text-lg font-bold text-[#1F2937]">{data.n_indice.toFixed(1)}</p>
                <p className="text-[10px] text-[#6B7280]">Índice Nutrición</p>
              </div>
              <div className="px-3 py-1.5 rounded-xl bg-white border border-[#E2E8F0] shadow-sm text-center">
                <p className="text-sm font-bold text-[#1F2937]">{data.n_nivel}</p>
                <p className="text-[10px] text-[#6B7280]">Nivel Nutrición</p>
              </div>
            </div>
          )}
        </div>
        ...
        {noEncuesta && (
          <div className="flex flex-col items-center gap-4 py-16 text-center">
            <Salad className="w-12 h-12 text-[#CBD5E1]" />
            <div>
              <p className="font-semibold text-[#1F2937]">Completa la encuesta primero</p>
              <p className="text-sm text-[#6B7280] mt-1">
                Debes completar el cuestionario de salud para ver tus recomendaciones de nutrición.
              </p>
            </div>
            <Link
          ...
          <>
            {data.tarjetas.length === 0 ? (
              <div className="text-center py-16 text-[#6B7280]">
                ...
                <p className="text-sm">¡Excelente! No tienes áreas de mejora en nutrición.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                {data.tarjetas.map((t, i) => (
                  <TarjetaN key={i} tarjeta={t} />
                ))}
              </div>
            )}
          </>
