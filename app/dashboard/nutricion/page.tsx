...
import { ChevronDown, ChevronUp, Users, Salad, AlertCircle, Building2, GraduationCap, Bell, Check, XCircle } from "lucide-react"
...
type NData = { total_usuarios: number; facultades: Facultad[] }
...
const N_ITEM_TEXTO: Record<string, string> = {
  n_item_02: "Consumir alimentos reducidos en grasas, grasas saturadas o calorías",
  n_item_08: "Limitar el consumo de alimentos con azúcares añadidos",
  n_item_14: "Comer diariamente de 6 a 11 porciones de carbohidratos",
  n_item_21: "Comer diariamente de 2 a 4 porciones de frutas",
  n_item_27: "Comer diariamente de 3 a 5 porciones de vegetales",
  n_item_33: "Comer diariamente de 2 a 3 porciones de lácteos",
  n_item_39: "Comer diariamente proteína de origen vegetal",
  n_item_40: "Comer diariamente proteína de origen animal",
  n_item_46: "Observar los sellos de advertencia en productos ultraprocesados",
  n_item_51: "Desayunar diariamente",
}
...
// El nivel llega de n_nivel, calculado por el backend con los cortes reales
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <ComparativoAnterior actual={n.n_indice} anterior={usuario.indice_anterior} />
          </div>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 pt-2 bg-[#F8FAFC] border-t border-[#E2E8F0]">
          <div className="sm:hidden mb-3">
            <p className="text-xs font-medium text-[#6B7280] mb-1">Índice Nutrición</p>
            <IndiceBar indice={n.n_indice} nivel={n.n_nivel} />
          </div>
          <div className="flex flex-col gap-1.5 mb-3">
            {N_ITEMS.map((item) => {
              const puntaje = n[item]
              ...
              return (
                <div key={item} className="flex items-center gap-3 bg-white border border-[#E2E8F0] rounded-lg px-3 py-2">
                  <span className="text-xs font-bold text-[#6B7280] w-14 shrink-0">Ítem {item.replace("n_item_", "")}</span>
                  <span className="flex-1 text-xs text-[#1F2937] leading-tight">{N_ITEM_TEXTO[item]}</span>
...
export default function NutricionPage() {
  ...
  useEffect(() => {
    ...
    if (!token) return
    api
      .get("/encuesta/nutricion/resultados/estadisticas")
  ...
  const fetchData = useCallback((f: Filtros) => {
    ...
    const url = `/encuesta/nutricion/resultados${params.toString() ? `?${params}` : ""}`
    api.get(url)
      .then((res) => {
        const d: NData = res.data
        setData(d)
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <DashboardNavbar role="nutricion" />
      <VolverAlPanelAdmin />
      ...
      <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6 flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold font-heading text-[#1F2937]">Nutrición</h1>
        ...
        {stats && <EstadisticasSection stats={stats} tituloDimension="Nutrición" />}
